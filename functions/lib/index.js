...
export const setCustomClaims = onCall({ region: 'asia-south1' }, async (request) => {
    const { uid, role, coachId } = request.data;
...
async function generateReceiptPdf(payment, paymentId) {
...
export const createInvoice = onCall({ region: 'asia-south1', secrets: [SUPPORT_EMAIL] }, async (request) => {
    ...
    const { coachId, items, total, currency, status, dueAt } = request.data;
...
export const sendInvoiceEmail = onCall({ region: 'asia-south1', secrets: [SUPPORT_EMAIL] }, async (request) => {
    ...
    const { invoiceId, email } = request.data;
    if (!invoiceId || !email) {
...
export const onInvitationCreated = onDocumentCreated({ document: 'invitations/{invitationId}', secrets: [GMAIL_EMAIL, GMAIL_PASSWORD] }, async (event) => {
